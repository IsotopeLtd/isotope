...
class _RegistrarImplementation implements Registrar {
  ...
  _ServiceFactory _findFactoryByNameOrType<T>(String instanceName) {
    ...
    assert(
      ...
      'Registrar: You have to provide either a type or a name. Did you accidentally do  `var sl=Registrar.instance();` instead of var sl=Registrar.instance;',
    );
    ...
    if (instanceName != null) {
      ...
      assert(instanceFactory != null,
          "Object/factory with name $instanceName is not registered with Registrar. Did you forget to register it?");
    } else {
      instanceFactory = _factories[T] as _ServiceFactory<T, dynamic, dynamic>;
      assert(instanceFactory != null,
          "No type ${T.toString()} is registered inside Registrar.\n Did you forget to pass an instance name? \n(Did you accidentally do  Registrar sl=Registrar.instance(); instead of Registrar sl=Registrar.instance;\did you forget to register it?)");
    }
  ...
  /// Callable class so that you can write `Registrar.instance<MyType>` instead of
  /// `Registrar.instance.get<MyType>`
  ...
  ///
  /// example:
  ///    registrar.registerFactoryParam<TestClassParam,String,int>((s,i) async
  ///        => TestClassParam(param1:s, param2: i));
  ///
  /// if you only use one parameter:
  ///
  ///    registrar.registerFactoryParam<TestClassParam,String,void>((s,_) async
  ///        => TestClassParam(param1:s);
  @override
      ...
      if (dependsOn?.isNotEmpty ?? false) {
        ...
        dependsOn.forEach((type) {
          ...
          throwIf(dependentFactory == null,
              ArgumentError('Dependent Type $type is not registered with Registrar'));
  ...
  /// Tests if an [instance] of an object or a type [T] or a name [instanceName]
  ...
  void resetLazySingleton<T>(
    ...
    throwIfNot(
        instanceFactory.factoryType == _ServiceFactoryType.lazy,
        StateError(
            'There is no type ${instance.runtimeType} registered as LazySingleton with Registrar'));
  ...
  _ServiceFactory _findFactoryByInstance(Object instance) {
    ...
    throwIf(
        ...
        StateError(
            'There is no object type ${instance.runtimeType} registered with Registrar'));

    throwIfNot(
        registeredFactories.length == 1,
        StateError(
            'Cannot register ${instance.runtimeType} more than once with Registrar'));
  ...
  /// the Singleton.
  /// If [instance] has a value registered will search for the responsible Singleton
  /// and completes all futures that [isReady] might be waiting for.
  ...
  Future<void> isReady<T>({
    ...
    throwIfNot(
      ...
      ArgumentError(
          'You only can use this function on Singletons that are async, that are marked as dependent '
