...
import 'package:isotope/src/view_models/base_view_model.dart';

enum _ViewModelBuilderType { NonReactive, Reactive }

/// A widget that provides base functionality for the MVVM-like provider architecture.
class ViewModelBuilder<T extends ChangeNotifier> extends StatefulWidget {
  final Widget staticChild;

  /// Fires once when the viewmodel is created or set for the first time. If you want this 
  /// to fire everytime the widget is inserted set [createNewModelOnInsert] to true.
  ...
  /// Indicates if you want Provider to dispose the viewmodel when it's removed from the 
  /// widget tree. Default is true.
  ...
  /// Constructs a viewmodel provider that will not rebuild the provided widget 
  /// when notifyListeners is called. Widget from [builder] will be used as a 
  /// static child and will not rebuild when notifyListeners is called.
  ...
  /// Constructs a viewmodel provider that fires the [builder] function when 
  /// notifyListeners is called in the viewmodel.
...
class _ViewModelBuilderState<T extends ChangeNotifier>
  ...
  void _initialiseSpecialViewModels() {
    // Add any additional actions here for specialised viewmodels.
